...
export async function POST(request: Request) {
  ...
  try {
    const { id, message, selectedChatModel, selectedVisibilityType, searchEnabled } =
      requestBody;
    ...
    const stream = createDataStream({
      execute: (dataStream) => {
        const result = streamText({
          ...
          tools: {
            ...(searchEnabled ? { web_search_preview: openai.tools.webSearchPreview({searchContextSize: 'high'}) } : {}),
            getWeather,
            ...
            if (userId) {
              try {
                ...
                console.log('💾 Saving assistant message:', {
                  ...
                  content: assistantMessage.parts?.[0]?.text?.substring(0, 100) + '...'
                });
